...
#+begin_src R :session *R* :tangle R/7z_checkout.R :exports none :eval no
#' @name 7z_checkout
...
#' @export
#'
7z_checkout <- function(
archive_id  <- "air_pollution_testing_checkin_to_q_drive"
dirlist <- 'Air_pollution_modelling_LUR_Western_Sydney/LUR_Western_Sydney_passive_samplers/" "projects/Air_Pollution_Monitoring_Stations_NSW/AP_monitor_NSW_2014_2015/'
  
  cmd <- sprintf('7z a -t7z versions/%s_%s.7z "%s"', archive_id, gsub(":", "-", gsub(" ", "-", Sys.time())),
                 dirlist)
  cat(cmd)
