...
#+begin_src R :session *R* :tangle ~/projects/ivanhanigan.github.com.raw/_posts/2013-10-13-dc-uploader-and-ANU-DataCommons.md :exports none :eval no :padline no
  ...
  change the anudc.conf: to test out the scripts by creating some sample records, please uncomment the “host” field in the file that points to dc7-dev2.anu.edu.au:8443 , and comment out the one that points to datacommons.anu.edu.au:8443  (NOTE THAT I COULDNT CREATE RECORDS IN THE DEV SERVER)
  ...
  <p></p>
  # go to the website
  Go to the production server at [This Link](https://datacommons.anu.edu.au:8443/DataCommons)
  This has created a new record in our account.
  
  Looking at metadata.txt it has been modified with the new pid.
  
  # Errors encountered on dev environment
  When I tried using [the dev site](https://dc7-dev2.anu.edu.au:8443/DataCommons/) I got the following errors:

      ## Creating record at dc7-dev2.anu.edu.au:8443/DataCommons/rest/display/new?layout=def:display&tmplt=tmplt:1 ...
  ...
  - I need to get the Keys.txt file that contains the full list of values that can be specified in this metadata.txt file.     
  - Dev server not working?  I tried from public, ncephgis VPN and nceph local IP addresses.
  - Hardcoded password is bad idea
  - how does dataset get sent?  
