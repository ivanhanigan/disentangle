...
#+begin_src R :session *R* :tangle ~/projects/ivanhanigan.github.com.raw/_posts/2013-11-20-sync-endnote-and-mendeley-using-r-xml.md :exports none :eval no :padline no
  #### R Code:
      endnote_mendeley_df <- function(input_xml, nrow_to_try = 1000){
  ...
  #### R Do-merge:
      ...
      endnote_not_in_mendeley <- endnote_not_in_mendeley[is.na(endnote_not_in_mendeley$V1.y),]
      nrow(endnote_not_in_mendeley)
      # 66 refs in endnote are not in mendeley
  
  #### TODO fuzzy matching
  - this relies on the author, date and title to be written the same in both
  - could  use levenshtein string matching to identify near misses?
...
#### Results
- I found that XML package in R can
