...
passwd newuser2
#+end_src
** secure passwords using R
This will require R is installed
*** genpw
this is probably not as secure
#+name:genpw
#+begin_src R :session *R* :tangle src/genpw.r :exports none :eval no
  ###########################################################################
  # newnode: genpw
  # random password
  genpw <- function(){
          pwd <- paste(paste(head(cbind(letters,sample(1:100,26))[order(cbind(letters,sample(1:100,26))[,2]),])[1:7],collapse=""),'#',sep="")
          pwd <- paste(toupper(substr(pwd,1,1)), substr(pwd, 2, nchar(pwd)),sep='')
          return(pwd)
          }
  pwd <- genpw()
  print(pwd)
