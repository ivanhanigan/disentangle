...
# name:swap
free -m | grep Swap
#Create an empty file called /swapfile from about the same amount of RAM (here just on GB but need to come back and review)
dd if=/dev/zero of=/swapfile bs=1024000 count=1024
#Format the new file to make it a swap file
mkswap /swapfile
#Enable the new swapfile. Only the swapon command is needed, but with the free command you can clearly see the swap space is made available to the system.
free -m | grep Swap
swapon /swapfile
free -m | grep Swap
# TO LAZY TO ADD TO BOOT SO JUST DO SWAPON EVERY TIME?
#+end_src
...
#+name:tassla01
#+begin_src R :session *R* :tangle src/tassla06.r :exports none :eval no
  ################################################################
  # name:tassla06
...
The postGIS database is called 'ewedb' and has a user 'gislibrary' with password 'gislibrary'.
...
*** ian and Steve
Hi Ian and Steve,
The prototype Virtual Lab "Swish-Brawn-And-Brains" service hosted on the Nectar Research Cloud is online. 
It is only a pilot study at the moment and I'd be keen to get you to test it for this use case if you are willing to?

The main portal is http://swish-climate-impact-assessment.github.com/
From which you should be able to navigate to the:

    data catalogue (and registry if you are an admin user)
    geoserver
    Rstudio server
    github repositories
    data management plan
    case studies/vignettes/tutorials


The Rstudio server is only secured by a self-signed certificate at the moment so you'll have to accept the first time -  also note that chrome and firefox are preferred.

I've added most of the security permissions you need and the one password should work for all features (Steve, can you wait till later as I have just finished Ian this arvo).

The postGIS database is called 'ewedb' and has a general user 'gislibrary' with password 'gislibrary'.
Your user has a dedicated schema for your own work.

Let us know if you use it and it works?

Cheers,
Ivan 'Swish-Data-Manager' Hanigan.
*** steve2
OK I think I have done all the bits:

1 Rstudio https://115.146.93.108

2 oraphi http://115.146.93.108:8080/f?p=102

3 ddiindex http://115.146.93.108:8181/ddiindex

4 ewedb = psql –h 115.146.94.209 –d ewedb –U steven_mceachern

Geoserver is not a priority for analysts and I guess I can just let you be the admin if you wanna publish?

So you can just point QGIS WFS tool at http://115.146.94.209:8181/geoserver/wfs

 

Don’t put too much data in there as I haven’t attached any storage and there is only about 3 GB spare (I shrank the swapspace against advice).

Also don’t try the RSS feed from http://swish-climate-impact-assessment.github.com yet as I haven’t updated it to the right one yet.

 

Let’s see… what have I forgotten?

I guess I need you to fill out a Data Management Plan Checklist (attached – just the ANU/ANDS Guidelines bit) and we can enshrine our relationship in a provision agreement?

I’ve set your access period to expire on 31/12/2012, I guess I can offer near-offline storage on CMBE servers for a period and then archived at (you guessed it) the ADA?
