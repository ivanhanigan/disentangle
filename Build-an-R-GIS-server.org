...
#+name:tassla01
#+begin_src R :session *shell* :tangle src/tassla01.r :exports none :eval no
  ################################################################
  # name:tassla01
  ...
  # plot(sd, add = T)
   names(sd2@data)
   writeOGR(sd2,'tassla01.shp','tassla01','ESRI Shapefile')
   test <- readOGR(dsn = 'tassla01.shp', layer = 'tassla01')
   plot(test, col = 'grey')
   rm(sd)
...
shp2pgsql -s 4283 -D tassla01.shp public.tassla01 > tassla01.sql
# psql -d pgisdb -U postgres -W -h 115.146.94.209 -f tassla01.sql
